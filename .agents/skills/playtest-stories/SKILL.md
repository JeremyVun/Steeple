...
the live copies are `src/schemas/`.)
...
this user gives up. Mirror the bundled `persona-exploratory.md` next to this
file (a copy of the built-in). `playtest new persona <name>` scaffolds one into
`./personas/` relative to cwd, so run it from the suite root; confirm names
resolve with `playtest personas` from the suite dir.
