...
- **Fake / spoofed listings.** **A human reviews a new host's first listing before it goes live** — the listing itself, the venue, and any ownership / lease-authority evidence the host supplied, weighed alongside the founder's own knowledge of the institution. That single decision both verifies the venue and publishes the listing; from then on the host is trusted and runs self-serve (a trusted host's later rooms and venues publish immediately). Evidence submission is an **input** to the review, not a precondition — the founder may verify a church out-of-band and approve on that basis. Steeple stores review metadata and links to externally hosted proof documents, not raw sensitive document contents. *(Single-gate model adopted 2026-08-05, superseding the earlier separate venue-verification + per-listing-approval steps — build: `docs/backlog/v2_migration/`.)*
...
```
  [ Flutter app ]      [ Web funnel: HTMX + light JS ]      [ HTMX admin ]
        │                          │                              │
        ▼                          ▼                              ▼
   Mobile edge API            Web edge API              admin (authelia at edge)
        └──────────────┬────────────┴────────────────┬────────────┘
...
- **Admin auth (separate from consumer SSO) — decided differently at build time:** the admin has **no in-app auth at all, by design**. Local accounts + TOTP via ASP.NET Core Identity were never built; access control is **authelia at the Caddy edge**, which gates the admin hostname and forwards `Remote-User` for audit attribution. Served on a **separate, non-public hostname** reachable only through that proxy. (Defence-in-depth local accounts were considered and **dropped** 2026-08-05 — authelia is the auth story; SYSTEM_DESIGN §17.)
