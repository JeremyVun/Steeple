...
| 2026-08-07 | **The web v2 production migration (D1–D9) is complete and closed by its P6 sweep.** The owner's five-step E2E was driven as three humans against the compose stack + a Development API (dev SSO/mailbox are Development-only by construction, so the sign-in steps ride a `build:debug` bundle on the dev API; the anonymous surface and API-down honesty were driven on the production bundle at :8080); `dotnet test` 403+96 green; every web suite green under its documented flags or its reds documented in its header. No architecture deviated in the sweep. Two config-level facts it surfaced, recorded rather than redesigned: (a) the 2026-08-06 dev-key guard now bites the local compose stack — a `.env` carrying the repository dev JWT key refuses to start in Production mode, which is the guard working; (b) media-origin CSP is the one place local compose renders uploaded photos blocked (web CSP is origin-relative, Admin's default is https-only) — folded into the standing "media base URL is final before real photos exist" open decision (`docs/backlog/README.md`) | The migration's own decisions live in `v2_migration/design.md` (kept); this row is the log's record that the plan executed without deviating from them, and where the sweep's residue landed |
