...
| 2026-07-04 | **Transactional email = Resend** behind `IEmailGateway` (plain-text, fire-and-forget after the inbox row is written; no-send without a key) | Free tier fits the cost ceiling; one HTTP adapter to swap (SES/Postmark are drop-in behind the port). Hardened 2026-08-06: unconfigured sends log no recipient, subject, or body |
...
| 2026-08-05 | **Upcoming-booking reminders run on a `BackgroundService`, the first and only background loop in the API** — a `booking_reminders` sent-ledger (unique `(OccurrenceId, Kind)`, claimed before dispatch) makes it idempotent; "coming up" fires T−7d for a booking's *first* upcoming occurrence only, "tomorrow" fires T−1d for *every* occurrence; both parties are notified through the existing dispatcher | The lazy-sweep-on-read decisions (2026-07-04) hold for anything a reader can observe, but a reminder's entire purpose is to reach someone who *isn't* looking — no read to hang it off. The asymmetric windows keep a weekly recurring booking from generating two emails every week for its whole term; the DB-level claim (not application bookkeeping) is what makes a restart, a second replica or a short test cadence safe |
