...
| **Manage** ✅ | provider self-service venue/room CRUD, venue-scoped moderation + durable operator takedowns | `IVenueManagerRepository`, `IManageRepository`, `IGeocodingGateway` |
...
`docs/contracts/seo.md` owns the crawler surface (sitemap, robots, canonicals, per-listing
meta, OG, JSON-LD); `docs/backlog/seo/` owns the unbuilt clean-route/listing-document work and
its deferrals (area landing pages, CWV). Architecture hooks it needs:
...
| 2026-07-04 | **Transactional email = Resend** behind `IEmailGateway` (plain-text; no-send without a key). **Delivery mechanics superseded 2026-08-09:** the original fire-and-forget call became the transactional outbox below. | Free tier fits the cost ceiling; one HTTP adapter to swap (SES/Postmark are drop-in behind the port). Hardened 2026-08-06: unconfigured sends log no recipient, subject, or body |
...
| 2026-08-05 | **Upcoming-booking reminders run on a `BackgroundService`, the first and only background loop in the API** — a `booking_reminders` sent-ledger (unique `(OccurrenceId, Kind)`, claimed before dispatch) makes it idempotent; "coming up" fires T−7d for a booking's *first* upcoming occurrence only, "tomorrow" fires T−1d for *every* occurrence; both parties receive inbox/push, while email is limited to T−1d for the booking's first occurrence | The lazy-sweep-on-read decisions (2026-07-04) hold for anything a reader can observe, but a reminder's entire purpose is to reach someone who *isn't* looking — no read to hang it off. Limiting email to the first session prevents a recurring booking becoming a permanent two-emails-per-week subscription; the DB-level claim (not application bookkeeping) is what makes a restart, a second replica or a short test cadence safe |
...
| 2026-08-05 | **Idempotent manage creates use a per-user ledger table, not the applications module's per-row column (D8, deviation).** `idempotency_records` PK `(UserId, Scope, Key) → ResourceId` (**new changeset 016**, where `v2_migration/design.md` §4 said no new changesets were needed) is written in the same `SaveChanges` as the venue/room it buys, so the primary key is the race guard. Applications keep their 004 column unchanged | Applications' filtered unique `(OrganizerId, IdempotencyKey)` works because an application carries its owner as a column; a venue does not — ownership lives in `venue_managers`, so the per-user uniqueness that makes the guard race-safe can't be expressed on `venues`. Adding a `CreatedByUserId` column purely to host the index would invent a second, weaker ownership fact beside `venue_managers`; scoping rooms by `(VenueId, Key)` would answer a co-manager's replay with someone else's room. The ledger generalizes to any future create for the cost of one small table |
| 2026-08-05 | **Web v2's correspondence is server-truth, and `store.js` is strictly its mirror (D4/D5, built).** One new client seam, `data/correspondence.js`, owns every read and write after a request is written (inbox, thread, withdraw, counter response, host decisions, the payments method-on-file); the store has one entry point per shape (`mirrorApplication` / `mirrorApplications` / `mirrorBooking` / `mirrorManagedVenues`) and no local status machine at all. Two deviations the build made explicit: (a) the **hosting entry point** is a routing decision, not a screen — signed out it opens sign-in, signed in with no managed venue it opens the listing flow, and a desk renders only for a manager of ≥1 real venue (the old switch opened a seeded church's desk, with its demo correspondence, to anybody who pressed it); (b) the **demo fixture stays in dev builds as village scenery** and is contained by construction rather than by a flag — its letters carry the seed's own ids, a real account's id is a GUID, and the desk is scoped to `GET /manage/venues` | A cache that can also decide things is two records, and the second one is always the one that lies to somebody. Making the mirror write-only-from-the-wire is what lets localStorage be cleared mid-flow without losing a fact — which is the acceptance test. The entry-point deviation is the same principle applied to the host side: a stranger shown a business is the demo seed deciding something about the world |
| 2026-08-06 | **The web refresh token moves to an httpOnly cookie, and rotation gets a 30s reuse grace.** `POST /auth/sessions` and `/auth/refresh` take an optional `refreshTransport: "body" \| "cookie"` (default `body`, so **mobile is untouched**); cookie mode omits the token from the JSON and sets `steeple_refresh` — HttpOnly, SameSite=Strict, Path=/, Max-Age = refresh TTL, Secure when the request is https (honouring `X-Forwarded-Proto`, since the edge terminates TLS). Web keeps the access token in module memory only, and localStorage holds nothing but the person and the reason their session last changed. `DELETE /auth/sessions` and `/me/sessions` now accept that cookie **instead of** a bearer, closing the documented "sign-out with a stale access token revokes nothing" gap. Refresh moves to its own per-IP limiter (60/min), off the shared `auth` budget. Reuse of a token **inside** 30s of its rotation is answered with the successor pair instead of revoking the family; outside that window, or for an older ancestor, the family still dies | Two tabs are one session: they 401 together and rotate together, and reuse detection was reading that as theft — a person with two tabs open was being signed out for using the product normally. The grace is enforced by two things at once: the DB revoke is now **conditional on the row still being unrevoked**, so exactly one caller ever writes a successor and one family can never fork into two live branches; and an **in-memory map** remembers {predecessor hash → successor pair} for the window, because tokens are stored hashed and the successor's raw value exists only in the process that minted it. **Rejected:** a schema-backed grace (a `successor_token` column, or storing the raw successor) — that puts a live raw credential in Postgres, which is the exact thing hashing exists to prevent, for a window measured in seconds. **Rejected:** raising the `auth` permit limit instead of splitting refresh out — the sign-in budget exists to slow guessing, and a refresh guesses nothing; leaving them shared would let a few tabs behind one NAT starve sign-in itself. The in-memory choice costs correctness only on a restart mid-race or a second API instance, where it degrades to today's behaviour (the losing tab is signed out) — never to a security hole, because the conditional update is still the arbiter |
...
| 2026-08-07 | **The web v2 production migration (D1–D9) is complete and closed by its P6 sweep.** The owner's five-step E2E was driven as three humans against the compose stack + a Development API (dev SSO/mailbox are Development-only by construction, so the sign-in steps ride a `build:debug` bundle on the dev API; the anonymous surface and API-down honesty were driven on the production bundle at :8080); `dotnet test` 403+96 green; every web suite green under its documented flags or its reds documented in its header. No architecture deviated in the sweep. Its dev-key guard finding remains intentional; its local media-origin CSP finding was resolved later that day by the origin-independent local-media decision below. | The migration's own decisions live in `v2_migration/design.md` (kept); this row is the log's record that the plan executed without deviating from them, and where the sweep's residue landed |
