...
void main() {
  group('StatusChip (DESIGN_SYSTEM §8.4)', () {
    ...
    testWidgets('unknown token renders humanized, never crashes', (
      tester,
    ) async {
      await tester.pumpWidget(
        host(
          const StatusChip(
            statusRaw: 'pendingReview',
            domain: StatusDomain.application,
          ),
        ),
      );
  ...
  group('ListingCard (DESIGN_SYSTEM §8.5)', () {
    ...
    testWidgets('listing shows the price badge and anatomy', (tester) async {
    ...
    testWidgets('fractional price renders with cents', (tester) async {
      await tester.pumpWidget(
        host(ListingCard(summary: summary(pricePerHour: 25.5))),
      );
      expect(find.text(r'$25.50/hr'), findsOneWidget);
    });
    ...
    testWidgets('whole card carries one semantic label', (tester) async {
      ...
      await tester.pumpWidget(
        host(ListingCard(summary: summary(), onTap: () {})),
      );
      expect(
        find.bySemanticsLabel(
          "Main Hall, St. Andrew's, \$25 per hour, seats 60",
        ),
        findsOneWidget,
      );
      handle.dispose();
  ...
  group('ErrorView (§8.7)', () {
    testWidgets('retryable error shows Try again; copy has no codes', (
      tester,
    ) async {
      var retried = false;
      await tester.pumpWidget(
    ...
    testWidgets('notFound reads as "no longer available", no retry', (
      tester,
    ) async {
      await tester.pumpWidget(
        host(
